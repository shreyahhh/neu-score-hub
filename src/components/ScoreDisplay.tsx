...
interface ScoreDisplayProps {
  result: GameResult | any; // Accept both formats
  gameType?: string; // Optional game type for transformation
}
...
export function ScoreDisplay({ result, gameType }: ScoreDisplayProps) {
  ...
  const gameResult: GameResult = result.competencies 
    ...
    : transformGameResult(result, gameType || 'unknown');
      ...
      <Card className="border-2 border-primary">
        ...
        <CardContent>
          <div className="text-center">
            <div className={`text-6xl font-bold ${getScoreColor(gameResult.finalScore)}`}>
              {gameResult.finalScore.toFixed(1)}
            </div>
            <div className="text-xl text-muted-foreground mt-2">
              {getScoreLabel(gameResult.finalScore)}
            </div>
            <div className="text-sm text-muted-foreground mt-1">
              {gameResult.gameName} {gameResult.difficulty && `- ${gameResult.difficulty}`}
            </div>
            {gameResult.rawData?.version_used && (
              <div className="text-xs text-muted-foreground mt-1">
                Scoring Version: {gameResult.rawData.version_used}
              </div>
            )}
          </div>
        </CardContent>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            {gameResult.competencies.map((competency, index) => (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                <tr className="font-bold">
                  ...
                  <td></td>
                  <td></td>
                  <td className="text-right text-primary">{gameResult.finalScore.toFixed(2)}</td>
                </tr>
              </tbody>
            </table>
          </div>
